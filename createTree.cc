...
int main(int argc, char *argv[]) {
  ...
  TVector3 TPSsource(0,-6600,0); //*** [mm] Position of scanning magnets, where proton angles are calculated from. Every position coming from GATE is in mm
  float D_t = 50.0; //*** [mm] distance between trackers
  ...
  std::string linep;
  std::ifstream TPSGen("../gate/PlanDescriptionToGate.txt"); //*** TPS-plan file from GATE
  double datap[3];
  int j=0;
  while(getline(TPSGen, linep)) {
    ...
    if (j==40) NbProtonsperSpot = datap[2]; //beam spot positions and weights start at line 40 in this file!
    j++;
  }  
  cout<<NbProtonsperSpot<<endl;
    
  //----------------------------
  //For handling the proton WEPL. These .dat files must be prepared beforehand! (You can find them in this github)
  //----------------------------
  //For getting the WEPL from the MC calculated energy difference
  std::string line;
  std::ifstream SPWater("Water_Proton.dat"); //file containing St.Pow. From the underlying Geant4 simulation (made with water ionization potential 78eV!)
  double data[2];
...
//----------------------Read data from Monte Carlo made root files--------------------------
  //Front tracker data (t11 is tracker one, t12 is tracker two BEFORE the phantom) 
  ...
  //Rear tracker data (t21 is tracker three, t22 is tracker four AFTER the phantom) 
...
//----------------------Compile new root file from Monte Carlo root files-----------------
  ...
  tOut->Branch("scatter_in",&scatter_in); //Scattering from front trackers (calculated later using Highland, but can be found using MC too...) 
  tOut->Branch("scatter_out",&scatter_out); //Scattering from rear trackers (calculated later using Highland, but can be found using MC too... Though there is an energy dependence!)
  tOut->Branch("eventID",&IDexit22);  
  tOut->Branch("WEPLideal",&WEPLideal); //Ideal wepl calculated using findWEPL function
  tOut->Branch("WEPLreal",&WEPLreal); //This is either the WEPLideal repeated if ideal trackers is used, or the WEPLideal spread out using a Gaussian
  ...
  //The proton is identified	  
   ...
   //Make sure we get the correct TPS spots from the plan description file
	  ...
	  //Project from iso-centre closer to tracker position, though I chose -100 mm to make sure I am outside the phantom 
    ...
    WEPLideal=findWEPL(Einit, Estop);  //Finding the ideal WEPL
      
    if(detectorVersion==1){ //ideal detector
      WEPLreal = WEPLideal;   
      scatter_in = 0.0; //No scattering in ideal trackers!
      scatter_out = 0.0;
	  
      p1x = atan((x12-x11)/D_t); //calculating angles given tracker hits
    ...
    if(detectorVersion==2){ // Bergen detector
      //Bergen properties
      s_pos =0.005; //[mm] position uncertainty on hits! From ALPIDE documentation    
      scatter_in = 0.0; //no front tracker!
      //Calculate Sigma_scatter_out  	    
      //Prepare for scattering calculation using Highland, only the material between the first tracker ALPIDE and the phantom is considered (See, Bopp et.al. and Krah et.al.)    
      matThick1 = 0.02; //cm of Carbon fiber
      X01 = 25.723; //cm Carbon radiation length
      matBudget1 = matThick1/X01;
      matThick2 = 0.002; //cm of Silicon
      X02 = 9.365; //cm Silicon radiation length
...
// Extract WEPL
////////////////////////////////////////////
double findWEPL(double Einit,double Estop){
  ...
  return WEPL*step; // to get WEPL in cm, divide this by 10;
}
