...
double s_pos_in = 0.0;//*** cm Position unc. in front tracking layers, 0 = ideal detector, 0.0005 = Bergen detector, 0.0228 = LomaLinda
double s_pos_out = 0.0;//*** cm Position unc. in rear tracking layers, 0 = ideal detector, 0.0005 = Bergen detector, 0.0228 = LomaLinda
...
#define IMAGENAME "FullPath_Double" // ***SET MANUALLY*** Name of image/radiography
...
struct Proton{
  ...
  Float_t scatter_out, scatter_in;
  Float_t weplReal; // Should be getting this from the DTC (probability distribution?)
};

void ComputeMLP(Proton *, TProfile2D *);
...
int main(int argc, char *argv[]){
  ...
  tt->SetBranchAddress("scatter_in_h",&Point.scatter_in);
  ...
  int NPerformed = 0;
  int binGlobal;
  double angleX, angleZ, deltaX, deltaZ, WEPLn;
 
  for(int i=0;i<NEntries;i++){//Loop over all protons again
	if(i%100000 == 0) cout<<i<<" "<<NPerformed<<endl;
    tt->GetEntry(i);   
     
    binGlobal = WEPLMean->FindBin(Point.x21, Point.z21); //Exact same bin for all the other histograms and binGlobal = angleMeanX->GetBin((angleMeanX->GetXaxis()->FindBin(Point.z21)), (angleMeanX->GetYaxis()->FindBin(Point.x21)));
...
void ComputeMLP(Proton *Point, TProfile2D* project2D){
    ...
    TVector3 p_old((Point->x12), (Point->y12), (Point->z12));

    //Initialization of the MLP variables to be used
    float X_mlp,Z_mlp, theta_X_mlp,theta_Z_mlp, X_mlp_sigma,Z_mlp_sigma;
    TVector3 p;
    
    //Iterator for the matrix operations
    int a;
    
    //Matrices    
    double T_in[4]; //Eq. 25 in Krah
      T_in[0] = 0; 
      T_in[1] = 1; 
      T_in[2] = -1/(d_T);
      T_in[3] = 1/(d_T);
    double T_in_transpose[4];
      T_in_transpose[0] = T_in[0];
      T_in_transpose[1] = T_in[2];
      T_in_transpose[2] = T_in[1];
      T_in_transpose[3] = T_in[3];
    double y_inx_d[2];
      y_inx_d[0] = Point->x12;
      y_inx_d[1] = tan((Point->p1x)); 
    double y_inz_d[2];
      y_inz_d[0] = Point->z12;
      y_inz_d[1] = tan((Point->p1z));    
    
    double T_out[4]; //Eq. 26 in Krah
      T_out[0] = 1; 
      T_out[1] = 0; 
      T_out[2] = -1/(d_T);
      T_out[3] = 1/(d_T);
    ...
    double y_outx_d[2];
      y_outx_d[0] = Point->x21;
      y_outx_d[1] = tan((Point->p2x));   
    double y_outz_d[2];
      y_outz_d[0] = Point->z21;
      y_outz_d[1] = tan((Point->p2z));
      
    double Sigma_in[4]={0}; //Used to be called sigma_beam and have the shape of eq.29 in Krah, now eq.28 for double-sided
      Sigma_in[0] = pow(s_pos_in, 2) * ( (T_in[0] * T_in_transpose[0]) + (T_in[1] * T_in_transpose[2]) );
      Sigma_in[1] = pow(s_pos_in, 2) * ( (T_in[0] * T_in_transpose[1]) + (T_in[1] * T_in_transpose[3]) );
      Sigma_in[2] = pow(s_pos_in, 2) * ( (T_in[2] * T_in_transpose[0]) + (T_in[3] * T_in_transpose[2]) );
      Sigma_in[3] = (pow(s_pos_in, 2) * ( (T_in[2] * T_in_transpose[1]) + (T_in[3] * T_in_transpose[3]) )) + pow(Point->scatter_in, 2); 
    ...
    double u_in = Point->y12+d_entry;
    double u_out = Point->y21-d_exit;
    double step_length = (u_out - u_in)/Nsteps;
    double posy=u_in+step_length;
    
// Step through until reach the exit depth
    while(posy<u_out){  //y is in beam direction
        ...
        R_0_transpose[0] = 1;
        R_0_transpose[1] = 0;
        R_0_transpose[2] = posy-u_in;
        R_0_transpose[3] = 1;

        R_1_inverse[0] = 1;
        R_1_inverse[1] = -(u_out-posy);
        R_1_inverse[2] = 0;
        R_1_inverse[3] = 1;
        R_1_inverse_transpose[0] = 1; 
        R_1_inverse_transpose[1] = 0;
        R_1_inverse_transpose[2] = -(u_out-posy);
        R_1_inverse_transpose[3] = 1;
        
        //need these for later: R_0*SSS_in, and R_1_inverse*SSS_out
        ...
        sy1 = Sigmay1(posy-u_in);
        st1 = Sigmat1(posy-u_in);
        sty1 = Sigmaty1(posy-u_in);
            
        sy2 = Sigmay2(u_out-u_in,posy-u_in);
        sty2 = Sigmaty2(u_out-u_in,posy-u_in);
        st2 = Sigmat2(u_out-u_in,posy-u_in);
